...
import com.bot.echo.constant.DriverCategory;
import com.bot.echo.enums.Gender;
import com.bot.echo.template.DriverLicenceTemplate;
import com.bot.echo.entity.User;
import com.bot.echo.service.UserService;
import com.bot.echo.service.impl.UserServiceImpl;
...
public class MainHandler {
    ...
    private DriverLicenceTemplate driverLicenceTemplate;
    ...
    public void handleStart(User user) {
        driverLicenceTemplate = new DriverLicenceTemplate();
        sender.send(SenderAction.typingOn(user));
        sender.send(QuickReplies.builder()
                        .user(user)
    ...
    public void handleFacebookName(User user, @Param("use") String use) {
        if (Boolean.parseBoolean(use)) {
            driverLicenceTemplate.setFirstName(messenger.getProfile(user.getChatId()).getFirstName());
            driverLicenceTemplate.setLastName(messenger.getProfile(user.getChatId()).getLastName());
            driverLicenceTemplate.setImgUrl(messenger.getProfile(user.getChatId()).getProfilePicture());
            driverLicenceTemplate.setId("AB" + Math.abs(ThreadLocalRandom.current().nextInt()));
            sender.send(SenderAction.typingOn(user));
    ...
    public void handleEnterName(User user, @Text String name) {
        if (!name.isEmpty()) {
            driverLicenceTemplate.setFirstName(name);
            sender.send(SenderAction.typingOn(user));
    ...
    public void handleEnterSurname(User user, @Text String surname) {
        if (!surname.isEmpty()) {
            driverLicenceTemplate.setLastName(surname);
            sender.send(SenderAction.typingOn(user));
            sender.send(user, "Please, enter your date of birth in dd/mm/yyyy format, for example: 04/05/1988", 2000);
            userService.changeState(user, ChatState.ENTER_DATE_OF_BIRTH);
        } else {
    ...
    public void handleEnterDateOfBirth(User user, @Text String date) {
        ...
        if (dateTester.test(date)) {
            driverLicenceTemplate.setDateOfBirth(date);
            sender.send(SenderAction.typingOn(user));
    ...
    public void handleEnterAllCategory(User user, @Text String listOfCategory) {
        if (!listOfCategory.isEmpty()) {
            ...
            driverLicenceTemplate.setDriverCategoryList(categories);
    ...
    public void handleUseFacebookEmail(User user, @Text String email) {
        ...
        if (emailTester.test(email)) {
            driverLicenceTemplate.setEmail(email);
            sender.send(SenderAction.typingOn(user));
            sender.send(QuickReplies.builder()
                            .user(user)
    ...
    public void handleMale(User user, @Param("gender") String gender) {
        if (gender.equals("male")) {
            driverLicenceTemplate.setSex(Gender.MALE);
        } else {
            driverLicenceTemplate.setSex(Gender.FEMALE);
        }
        driverLicenceTemplate.setFinish(true);
        showAllEnteredData(user);
    }

    private void showAllEnteredData(User user) {
        sender.send(SenderAction.typingOn(user));
        sender.send(user, driverLicenceTemplate.toString(), 2000);
    }
...
////                        .title("TITLE")
////                        .subtitle("SUBTITLE")
////                        .imageUrl(messenger.getProfile(user.getChatId()).getProfilePicture())
////                        .button(new PostbackButton("myTitle", "payload1"))
////                        .build())
////                .build());
//
//
...
////                .addButton(new PostbackButton("myTitle", "payload1"))
...
////                .user(user)
////                .element(new ImageElement("https://picsum.photos/id/237/200/300"))
////                .build();
////
////        sender.send(request);
//
...
//    @Postback("payload1")
//    public void handleDefaultText(MessengerUser user, @Text String message) {
//        String youTyped = "you typed " + message;
//        sender.send(SenderAction.typingOn(user));
//        sender.send(user, youTyped);